<?php 
	...
	function uploadPhotos($count){
		...
		if(move_uploaded_file($photo_loc, $folder.$finalPhoto)){
			return "http://www.goodscab.com/login/".$folder.$finalPhoto;
		}else{
			return '';
		}
	}
	...
	if($resultQuery == 1){
		header("Location:MTSProductsCreate.php?msg1=MTS Product has been added to the Products List. Product ID is $id");
	}else{
		header("Location:MTSProductsCreate.php?msg=failed to create product");
	}
?>